    ...
    <head>
       ...
       <script>
		   ...
		   function tableHighlight(){
			   $('.data-table-table').on('click', 'tbody tr', function(event) {	  
...
//				 var id = document.getElementsByClassName("highlight").value - 1;
...
//				 $('#delete-btn').on('click', function(){
					...
					//for(var i = 1; i < table.rows.length; i++){
						//var isRow = table.rows[i];
						
						//	table.hasClass("highlight").deleteRow();
						//	i--;
...
//				 });
			...
			function tableCall{
				var name = document.getElementById("data-data-table");
				if(name){
					$.ajax({
						type: 'post',
						url: 'website.php',
						...
						success: function(response){
							$('#data-data-table').html(response);
						}
					});
				} else{
					$('#data-data-table').html("WRONG");
				}
    ...
    <body>
        ...
        <!-- Pushy Menu -->
        <nav class="pushy pushy-left" id="pushy-left" data-focus="#first-link">
	...
	<div class="data-table-container" autofocus>
...
-->
			...
			<button class="glyphicon glyphicon-plus-sign" onClick="window.open('http://localhost/Capstone%20Website/Form-add-row1.html', '_blank', 'toolbar=no,scrollbars=no,resizable=yes,top=200,left=500,width=500,height=300')" 	
				role="button"></button>
